...
void GetPredBox(int side, int num_object, int num_class, const Dtype* input_data,
            map<int, vector<BoxData> >* pred_boxes, bool use_sqrt, bool constraint, 
...
void EvalDetectionLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    GetPredBox(side_, num_object_, num_class_, input_data + input_index, &pred_boxes, sqrt_, constraint_, score_type_, nms_);
